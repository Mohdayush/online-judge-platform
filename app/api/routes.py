...
from app.schemas.contracts import ContestCreate, ContestResponse, LeaderboardEntry, LoginRequest, ProblemCreate, ProblemResponse, RegisterRequest, SubmissionCreate, SubmissionResponse, TestCaseCreate, TestCaseResponse, TokenResponse
...
def create_submission(payload: SubmissionCreate, db: Session = Depends(get_db), user: User = Depends(current_user)):
    ...
    if payload.contest_id:
        ...
        mapped_problem = db.scalar(select(ContestProblem).where(ContestProblem.contest_id == payload.contest_id, ContestProblem.problem_id == payload.problem_id))
        now = datetime.utcnow()
        if not contest or not registered or not mapped_problem:
            raise HTTPException(status_code=403, detail="You are not eligible to submit this contest problem")
        if not contest.starts_at <= now <= contest.ends_at:
            raise HTTPException(status_code=409, detail="Contest is not currently active")
...
def create_contest(payload: ContestCreate, db: Session = Depends(get_db), user: User = Depends(admin_user)):
    ...
    if len(problem_ids) != len(set(problem_ids)):
        raise HTTPException(status_code=422, detail="A problem can appear only once in a contest")
    found = db.scalars(select(Problem.id).where(Problem.id.in_(problem_ids))).all()
    if len(found) != len(problem_ids):
        raise HTTPException(status_code=404, detail="One or more problems do not exist")
...
def contest_leaderboard(contest_id: int, db: Session = Depends(get_db)):
    ...
    scores = {user.id: {"username": user.username, "score": 0, "solved": set(), "penalty": 0} for _, user in registrations}
    for submission, user in accepted:
        entry = scores.get(user.id)
        if entry is None or submission.problem_id in entry["solved"]:
            continue
        entry["solved"].add(submission.problem_id)
        entry["score"] += points.get(submission.problem_id, 0)
        entry["penalty"] += max(0, int((submission.created_at - contest.starts_at).total_seconds()))
    ordered = sorted(scores.values(), key=lambda item: (-item["score"], -len(item["solved"]), item["penalty"], item["username"]))
    return [LeaderboardEntry(rank=index + 1, username=item["username"], score=item["score"], solved=len(item["solved"]), penalty_seconds=item["penalty"]) for index, item in enumerate(ordered)]
